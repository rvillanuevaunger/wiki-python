...
for header in soup.find_all(['h1', 'h2', 'h3']):  # 1.
    ...
    for elem in header.next_siblings:  # 5.
        if elem.name and elem.name.startswith('h'):
            break
        if elem.name in ('p', 'ul'):  # 5.
            text = elem.get_text()  # 6.
            words = re.findall(r'\b\w+', text)  # 7.
            lower_case = Counter([word.lower() for word in words if not word.lower() in stop_words])  # 8.
